...
function extrinsic_uncoded = URC2_decoder_bcjr(apriori_uncoded, apriori_encoded)
    ...
    for bit_index = 2:length(apriori_uncoded)        
        temp = alphas(transitions(:,1),bit_index-1)+gammas(:,bit_index-1);  % 3 additions
    ...
    for bit_index = length(apriori_uncoded)-1:-1:1
        temp = betas(transitions(:,2),bit_index+1)+gammas(:,bit_index+1);  % 3 additions
        for state_index = 1:state_count
    ...
    deltas = alphas(transitions(:,1),:) + betas(transitions(:,2),:) + gammas_encoded;  % 6 additions
    
    % Calculate the uncoded extrinsic LLRs
    log_p0=maxstar(deltas(transitions(:,3) == 0,:));
    log_p1=maxstar(deltas(transitions(:,3) == 1,:));
    extrinsic_uncoded = log_p1-log_p0;  % 1 addition
