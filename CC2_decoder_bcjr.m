...
function [extrinsic_uncoded, extrinsic_encoded] = CC2_decoder_bcjr(apriori_uncoded, apriori_encoded)
...
transitions =  [1,          1,          0,          0,          0;
    ...
    for codebit_index = 1:size(apriori_encoded,1)
        gammas(transitions(:,3+codebit_index)==1,:) = gammas(transitions(:,3+codebit_index)==1,:) + repmat(apriori_encoded(codebit_index,:), sum(transitions(:,3+codebit_index)==1),1); % 4 additions
    end
    ...
    for bit_index = 2:length(apriori_uncoded)        
        temp = alphas(transitions(:,1),bit_index-1)+gammas(:,bit_index-1); % 7 additions
        for state_index = 1:state_count
            alphas(state_index,bit_index) = maxstar(temp(transitions(:,2) == state_index)); % 4 maxstar
        end
    end
    ...
    for bit_index = length(apriori_uncoded)-1:-1:1
        temp = betas(transitions(:,2),bit_index+1)+gammas(:,bit_index+1); % 7 additions
        for state_index = 1:state_count
            betas(state_index,bit_index) = maxstar(temp(transitions(:,1) == state_index)); % 4 maxstar
        end
    end

    % Calculate a posteriori transition log-probabilities
    deltas = alphas(transitions(:,1),:) + betas(transitions(:,2),:) + gammas; % 8 additions
    ...
    for codebit_index = 1:size(apriori_encoded,1)
        ...
        log_p1=maxstar(deltas(transitions(:,3+codebit_index) == 1,:)); % 8 maxstars
        extrinsic_encoded(codebit_index,:) = log_p1-log_p0-apriori_encoded(codebit_index,:); % 4 additions
    end
